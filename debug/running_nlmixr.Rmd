...
library(nlmixr2)
str(theo_sd)
#rite.csv(theo_sd, 'theo_nlmixr2.csv')
```
...
```{r}
fit_focei <- nlmixr(one.cmt, theo_sd, est="foce",
              control=foceiControl(interaction=TRUE, print=0))
...
```{r}
fit_foce <- nlmixr(one.cmt, theo_sd, est="foce",
...
```{r}
fit_fo <- nlmixr(one.cmt, theo_sd, est="fo",
                   control=foceiControl(interaction=FALSE, print=0))
...
``` {r}
pop_params <- list(
  theta = fixef(fit_foce),  # Named vector of fixed effects
  omega = fit_foce$omega,   # The full Omega matrix
  sigma = fit_foce$parFixedDf["add.sd", "Est."] # The residual error
)

# 3. Extract the individual EBEs
individual_ebes <- fit_foce$eta

# 4. View the extracted components
print("Population Theta values:")
print(pop_params$theta)

print("Omega Matrix:")
print(pop_params$omega)

print("Residual Error (sigma):")
print(pop_params$sigma)

print("Individual EBEs (first 6 subjects):")
print(head(individual_ebes))
...
```{r}
fit2 <- nlmixr(one.compartment, theo_sd,  est="fo",
...
fit2 <- nlmixr(one.compartment, theo_sd,  est="foce",
               control=list(print=0))
print(fit2)
```
