...
def arbitrary_objective_function(
    params,
    data,
    model_function=one_compartment_model,
    subject_id_c="SUBJID",
    conc_at_time_c="DV",
    time_c="TIME",
    ...
    predictions = []
    subject_iter_obj = (
        tqdm(data[subject_id_c].unique()) if verbose else data[subject_id_c].unique()
    )
    for subject in subject_iter_obj:
        ...
        for model_param in betas:  # for each of the coeff to be input to the model
            ...
            for allometic_effect in allometric_effects:
                subject_coeff[model_param] = (
                    subject_coeff[model_param] * allometic_effect
                )
                subject_coeff_history.append(subject_coeff)
        subject_coeffs_history[subject] = subject_coeff_history
        subject_coeff = {
            model_param: np.exp(subject_coeff[model_param])
            for model_param in subject_coeff
        }
        subject_coeff = [subject_coeff[i] for i in subject_coeff]
        sol = solve_ivp(
            model_function,
            [subject_data[time_c].min(), subject_data[time_c].max()],
            [initial_conc],
            t_eval=subject_data[time_c],
            args=(*subject_coeff, 1),
        )
...
def objective_function__mgkg_age(
    params,
    data,
    subject_id_c="SUBJID",
    dose_c="DOSR",
    ...
    float: The sum of squared errors (SSE).
    """
    k_pop, Vd_pop, k_beta_age, k_beta_mgkg, Vd_beta_age, Vd_beta_mgkg = (
...
def stack_ivp_predictions(
    ivp_predictions, time_c="TIME", pred_DV_c="Pred_DV", subject_id_c="SUBJID"
):
    dfs = []
...
def merge_ivp_predictions(
    df, ivp_predictions, time_c="TIME", pred_DV_c="Pred_DV", subject_id_c="SUBJID"
):
    df = df.copy()
